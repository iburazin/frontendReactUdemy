  ...
  function ourReducer(draft, action) {
    switch (action.type) {
      case "usernameImmediately":
        (draft.username.hasErrors = false), (draft.username.value = action.value);
      ...
      case "usernameAfterDelay":
        if (draft.username.value.length < 3) {
          (draft.username.hasErrors = true), (draft.username.message = "Username must be at least 3 characters.");
        }
      ...
      case "emailImmediately":
        (draft.email.hasErrors = false), (draft.email.value = action.value);
        return;
      case "emailAfterDelay":
        if (!/^\S+@\S+$/.test(draft.email.value)) {
          (draft.email.hasErrors = true), (draft.email.message = "You must provie a valid email address.");
        }
      ...
      case "emailUniqueResults":
        if (action.value) {
          (draft.email.hasErrors = true), (draft.email.isUnique = false), (draft.email.message = "That email is already bein used.");
        } else {
          draft.email.isUnique = true;
        }
        return;
      case "passwordImmediately":
        (draft.password.hasErrors = false), (draft.password.value = action.value);
        if (draft.password.value.length > 50) {
          (draft.password.hasErrors = true), (draft.password.message = "Password cannot exceed 50 characters.");
        }
        return;
      case "passwordAfterDelay":
        if (draft.password.value.length < 12) {
          (draft.password.hasErrors = true), (draft.password.message = "Password must be at least 12 characters.");
        }
        return;
      case "submitForm":
